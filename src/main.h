...
public:
    ...
    {
        // upscale: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // upscale: get transaction coin age
...
public:
    ...
    // upscale: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // upscale: two types of block: proof-of-work or proof-of-stake
    ...
    // upscale: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // upscale: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // upscale: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // upscale: block index flags
    enum  
    {
